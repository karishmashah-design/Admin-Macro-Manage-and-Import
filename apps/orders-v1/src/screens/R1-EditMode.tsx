...
import { Button, IconButton, Icon, Checkbox, TextField, Chip } from "@ds/ui";
...
export default function R2EditMode() {
  ...
  const [diagEditMode, setDiagEditMode] = useState(false);
      ...
      <div className="max-w-[640px] w-full px-[20px] py-[32px] flex flex-col gap-[24px]">
        ...
        <section>
          <div className="flex items-center justify-between mb-[16px]">
            ...
            <Button
              variant={diagEditMode ? "secondary" : "tertiary-neutral"}
              size="small"
              prefix={diagEditMode ? <Icon name="check" size={16} /> : <Icon name="edit" size={16} />}
              onClick={() => { setDiagEditMode((v) => !v); setPopover(null); }}
            >
              {diagEditMode ? "Done" : "Edit"}
            </Button>
          </div>
          ...
          <div className="flex flex-col gap-[4px] mb-[24px]">
                ...
                {diagEditMode ? (
                  <>
                    ...
                    <IconButton
                      ...
                      onClick={(e) => { e.stopPropagation(); setIcd10((prev) => prev.filter((x) => x.code !== c.code)); }}
                      aria-label="Remove"
                    />
                  </>
                ) : (
            ...
            {diagEditMode && (
              <Button variant="tertiary" size="small" prefix={<Icon name="add" size={16} />} onClick={(e) => openPopover(e, "icd10")} className="self-start">Add ICD-10 code</Button>
            )}
          </div>
          ...
          <div className="flex flex-col gap-[4px]">
                ...
                {diagEditMode ? (
                  <>
                    ...
                    <IconButton
                      size="small" variant="tertiary"
                      icon={<Icon name="close" size={16} />}
                      onClick={(e) => { e.stopPropagation(); setCpt((prev) => prev.filter((x) => x.code !== c.code)); }}
                      aria-label="Remove"
                    />
                  </>
                ) : (
            ...
            {diagEditMode && (
              <Button variant="tertiary" size="small" prefix={<Icon name="add" size={16} />} onClick={(e) => openPopover(e, "cpt")} className="self-start">Add CPT code</Button>
            )}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-col gap-[8px]">
            {/* Individual orders */}
            {orders.map((o) => (
              <div key={o.id} className="flex items-center gap-[4px]">
                {diagEditMode ? (
                  <>
                    <TextField
                      value={o.baseLabel ?? o.label}
                      readOnly
                    ...
                    {o.company && (
                      <TextField
                        value={o.company}
                        readOnly
                        className="w-[90px] shrink-0 cursor-pointer"
                        onClick={(e) => openPopover(e, "order-company", o.id)}
                      />
                    )}
                    {o.relatedIcd ? (
                      <TextField
                        value={o.relatedIcd}
                        readOnly
                        className="w-[72px] shrink-0 cursor-pointer"
                  ...
                  <div className="ml-[12px] pl-[12px] border-l-2 border-[#ebebeb] flex flex-col gap-[4px]">
                    {set.children.map((child) => (
                      <div key={child.id} className="flex items-center gap-[4px] min-h-[28px]">
                        ...
                        {diagEditMode ? (
                          <>
                            <TextField value={child.label} readOnly className="flex-1 min-w-0" />
                            <TextField value={child.company} readOnly className="w-[90px] shrink-0" />
                          </>
                        ) : (
                          <div className="flex items-center gap-[6px] h-[28px] px-[8px]">
            ...
            {diagEditMode && (
              <Button variant="tertiary" size="small" prefix={<Icon name="add" size={16} />} onClick={(e) => openPopover(e, "order")} className="self-start">Add order</Button>
            )}
          </div>
        </section>
