...
// ─── Component ──────────────────────────────────────────────────────────────────
// Direction A: Two zones — confidence indicator lives on the Dx group header;
// pre-charted orders are fully separated below a divider as a read-only "In chart" zone.

export default function R3DxA() {
  ...
  const { suggestedGroups, prechartedGroups, unlinkedSuggested } = useMemo(() => {
    const sugMap = new Map<string, FlatOrder[]>();
    const preMap = new Map<string, FlatOrder[]>();
    const unlinked: FlatOrder[] = [];
    for (const o of flatOrders) {
      const map = o.precharted ? preMap : sugMap;
      if (o.relatedIcd) {
        if (!map.has(o.relatedIcd)) map.set(o.relatedIcd, []);
        map.get(o.relatedIcd)!.push(o);
      } else if (!o.precharted) {
        unlinked.push(o);
      }
    }
    const toGroups = (m: Map<string, FlatOrder[]>) => {
      const out: Array<{ code: string; label: string; orders: FlatOrder[] }> = [];
      for (const c of icd10) {
        const ords = m.get(c.code);
        if (ords?.length) out.push({ code: c.code, label: c.providerLabel ?? c.description, orders: ords });
      }
      for (const [code, ords] of m) {
        if (!icd10.find((c) => c.code === code)) {
          const e = icd10Pool.find((c) => c.code === code);
          out.push({ code, label: e?.providerLabel ?? e?.description ?? code, orders: ords });
        }
      }
      return out;
    };
    return { suggestedGroups: toGroups(sugMap), prechartedGroups: toGroups(preMap), unlinkedSuggested: unlinked };
  }, [flatOrders, icd10]);
    ...
    <ScribeLayout activeTab={activeTab} onTabChange={setActiveTab}>
      <div className="max-w-[640px] w-full px-[20px] py-[32px] flex flex-col gap-[24px]">
        ...
        <section>
          ...
          {/* Suggested orders — grouped by Dx, confidence on header */}
          <div className="flex flex-col gap-[16px]">
            {suggestedGroups.map(({ code, label, orders }) => {
              const isConfidentGroup = orders.some((o) => o.confidence === "high");
              return (
          ...
          {/* In chart zone — read-only, dimmed */}
