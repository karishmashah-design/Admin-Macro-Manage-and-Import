...
// Company baked into the order title as "(Company)". Each interactive element
// has its own hover zone. ICD code shown as a brand chip, always visible.

type PopoverTarget = {
  rect: DOMRect;
  list: "icd10" | "cpt" | "order" | "order-icd" | "set-title" | "set-order" | "set-child-icd";
  code?: string;
  setId?: string;
};

export default function R2HoverReveal() {
            ...
            {orderSets.map((set) => {
              ...
              return (
                <div key={set.id} className="flex flex-col gap-[4px]">
                  {/* Set header — company baked into title (Co.Title variant) */}
